...
export async function readInstalledProducts(
  clientIdOverride?: string,
) {
  const {
    url,
  ...
  try {
    result = JSON.parse(
      text,
    ) as RemoteProductsResponse;
  } catch {
    throw new Error(
      "El backend del instalador devolvió productos en un formato inválido.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      result.message ||
        "No fue posible cargar los productos del cliente.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "El catálogo recibido pertenece a otro CLIENT_ID.",
    );
  }
