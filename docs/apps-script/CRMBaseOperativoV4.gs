const CRM_BASE_OPERATIVO_VERSION = "1.4.0";
...
function doGet(e) {
  try {
    ...
    const requestedClientId = String((e && e.parameter && e.parameter.clientId) || "").trim();
    validarClientIdOperativo_(requestedClientId);
...
function doPost(e) {
  try {
    ...
    validarClientIdOperativo_(rawPayload.clientId);
    ...
    if (action === "reenviarCotizacion") return jsonOperativo_(reenviarCotizacionOperativa_(payload));
    ...
    if (action === "buscarCotizacion") {
      ...
      return cotizacion
        ? jsonOperativo_({ ok: true, clientId: getClientIdOperativo_(), data: cotizacion })
        : jsonOperativo_({
            ok: false,
            clientId: getClientIdOperativo_(),
...
function validarClientIdOperativo_(clientIdRecibido) {
  const esperado = getClientIdOperativo_();
  const recibido = String(clientIdRecibido || "").trim();
  if (!recibido) throw new Error("CLIENT_ID_REQUERIDO");
  if (recibido !== esperado) throw new Error("CLIENT_ID_INCORRECTO");
  return true;
}

function sanitizarPayloadOperativo_(value) {
  if (Array.isArray(value)) return value.map(sanitizarPayloadOperativo_);
...
function registrarVentaOperativa_(payload) {
  ...
  lock.waitLock(30000);
  try {
    validarClientIdOperativo_(payload.clientId);
...
function crearCotizacionOperativa_(payload) {
  ...
  try {
    ...
    const envio = enviarCotizacionCorreoOperativa_({
      numero: numero,
    ...
    if (envio.ok) {
      ...
      actualizarEstadoCotizacionPorFilaOperativa_(quoteRow, estado, new Date());
    } else {
      mensaje = "Cotización generada correctamente. No fue posible enviar el correo.";
    }
...
function reenviarCotizacionOperativa_(payload) {
  const lock = LockService.getScriptLock();
  lock.waitLock(30000);
  try {
    validarClientIdOperativo_(payload.clientId);
    ...
    const email = String(payload.email || cotizacion.email || "").trim();
    ...
    const envio = enviarCotizacionCorreoOperativa_({
      ...
      pdfFile: getPdfFileDesdeUrlOperativa_(cotizacion.pdfUrl),
      pdfUrl: cotizacion.pdfUrl,
      documentoUrl: cotizacion.documentoUrl,
    });

    if (envio.ok) {
      actualizarEstadoCotizacionPorFilaOperativa_(cotizacion.fila, "ENVIADA", new Date());
    }
    ...
    return {
      ...
      data: {
        numero: cotizacion.numero,
        estado: envio.ok ? "ENVIADA" : cotizacion.estado,
        pdfUrl: cotizacion.pdfUrl,
        documentoUrl: cotizacion.documentoUrl,
        envio: envio,
        mensaje: envio.ok
          ? "Cotización reenviada correctamente."
          : "No fue posible reenviar el correo. La cotización se conserva sin cambios.",
      },
    };
  } finally {
    lock.releaseLock();
...
function enviarCotizacionCorreoOperativa_(data) {
  try {
    const email = String(data.email || "").trim();
    if (!email || email.indexOf("@") < 1) {
      return { ok: false, canal: "EMAIL", error: "EMAIL_INVALIDO", mensaje: "Correo inválido." };
    }

    if (!data.pdfFile) {
      return { ok: false, canal: "EMAIL", destinatario: email, error: "PDF_NO_DISPONIBLE", mensaje: "El PDF no está disponible." };
    }

    const company = obtenerEmpresaInstaladaOperativa_();
    const companyName = String(company.name || company.legalName || "Empresa");
    const body = [
      "Hola " + String(data.cliente || "") + ",",
      "",
      "Adjuntamos la cotización " + String(data.numero || "") + " por " + formatClpOperativo_(data.total) + ".",
      "",
      data.pdfUrl ? "PDF: " + data.pdfUrl : "",
      data.documentoUrl ? "Documento: " + data.documentoUrl : "",
      "",
      "Saludos,",
      companyName,
    ].filter(String).join("\n");
    ...
    MailApp.sendEmail({
      to: email,
      subject: "Cotización " + String(data.numero || "") + " - " + companyName,
      body: body,
      htmlBody:
        "<p>Hola " + escapeHtmlOperativo_(String(data.cliente || "")) + ",</p>" +
        "<p>Adjuntamos la cotización <strong>" + escapeHtmlOperativo_(String(data.numero || "")) +
        "</strong> por <strong>" + escapeHtmlOperativo_(formatClpOperativo_(data.total)) + "</strong>.</p>" +
        (data.pdfUrl ? "<p><a href=\"" + escapeHtmlOperativo_(data.pdfUrl) + "\">Abrir PDF</a></p>" : "") +
        "<p>Saludos,<br>" + escapeHtmlOperativo_(companyName) + "</p>",
      attachments: [data.pdfFile.getBlob().setName("Cotizacion-" + String(data.numero || "") + ".pdf")],
      name: companyName,
    });
    ...
    return {
      ok: false,
      canal: "EMAIL",
      destinatario: String(data.email || ""),
...
function actualizarEstadoCotizacionPorFilaOperativa_(rowNumber, estado, now) {
  const sheet = getSheetOperativa_(CRM_BASE_SHEETS.COTIZACIONES);
  const header = headerMapOperativo_(sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0]);
  sheet.getRange(rowNumber, header.ESTADO + 1).setValue(estado);
  sheet.getRange(rowNumber, header.UPDATED_AT + 1).setValue(now || new Date());
}

function getPdfFileDesdeUrlOperativa_(url) {
  try {
    const id = extractDriveIdOperativo_(url);
    return id ? DriveApp.getFileById(id) : null;
  } catch (error) {
    console.error("No fue posible recuperar PDF", error);
    return null;
  }
}

function extractDriveIdOperativo_(value) {
  const text = String(value || "");
  const match = text.match(/[-\w]{20,}/);
  return match ? match[0] : "";
}

function escapeHtmlOperativo_(value) {
  return String(value || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function generarDocumentoCotizacionOperativa_(data) {
  try {
    ...
    if (data.descuento > 0) {
      body.appendParagraph("Descuento: -" + formatClpOperativo_(data.descuento));
    }
...
function jsonErrorOperativo_(error) {
  return jsonOperativo_({
    ok: false,
    clientId: safeClientIdOperativo_(),
    error: error && error.message ? error.message : String(error),
    mensaje: error && error.message ? error.message : String(error),
  });
}

function safeClientIdOperativo_() {
  try {
    return getClientIdOperativo_();
  } catch (error) {
    return "";
  }
}
