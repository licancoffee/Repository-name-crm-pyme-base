const CRM_BASE_OPERATIVO_VERSION = "1.3.0";
...
function crearCotizacionOperativa_(payload) {
  ...
  try {
    ...
    const estado = "GENERADA_SIN_ENVIO";
    const mensaje = "Cotización generada correctamente.";
...
function generarDocumentoCotizacionOperativa_(data) {
  try {
    ...
    const pdfBlob = docFile.getBlob()
      .getAs(MimeType.PDF)
      .setName("Cotizacion-" + data.numero + ".pdf");
...
function getOperationalSpreadsheetOperativo_() {
  const explicitId = PropertiesService.getScriptProperties()
