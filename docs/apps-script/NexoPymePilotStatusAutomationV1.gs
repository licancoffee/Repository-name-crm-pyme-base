const NEXO_PILOT_STATUS_AUTOMATION_VERSION = "1.0.0";

/**
 * NexoPyme — Automatización de estados de pilotos
 *
 ...
 * el estado comercial del piloto en la planilla NexoPyme - Prospectos y Pilotos.
 */
function actualizarEstadosPilotosNexoPyme() {
  ...
  if (lastRow < 2) {
    return {
      ok: true,
      version: NEXO_PILOT_STATUS_AUTOMATION_VERSION,
      actualizados: 0,
    };
  }
  ...
  values.forEach(function(row, index) {
    ...
    if (!(vence instanceof Date) || isNaN(vence.getTime())) {
      return;
    }

    // PREPARACION y CONVERTIDO son estados de decisión manual.
    if (estadoActual === "PREPARACION" || estadoActual === "PREPARACIÓN" || estadoActual === "CONVERTIDO") {
      return;
    }
    ...
    // El contador permanece como fórmula dinámica para mostrar días restantes.
 ...
 */
function instalarTriggerEstadosPilotosNexoPyme() {
  eliminarTriggersEstadosPilotosNexoPyme_();

  ScriptApp.newTrigger("actualizarEstadosPilotosNexoPyme")
    .timeBased()
    .everyDays(1)
    .atHour(0)
    .create();

  actualizarEstadosPilotosNexoPyme();

  return {
    ok: true,
    version: NEXO_PILOT_STATUS_AUTOMATION_VERSION,
    mensaje: "Trigger diario instalado.",
  };
}

function eliminarTriggersEstadosPilotosNexoPyme_() {
  ScriptApp.getProjectTriggers().forEach(function(trigger) {
    if (trigger.getHandlerFunction() === "actualizarEstadosPilotosNexoPyme") {
