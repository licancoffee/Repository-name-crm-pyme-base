const NEXO_PILOT_GENERATOR_VERSION = "1.0.0";

const NEXO_PILOT_HEADERS = {
...
function doGet() {
  return HtmlService.createHtmlOutput(getPilotGeneratorHtml_())
    .setTitle("NexoPyme - Generador de Pilotos")
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

function crearPilotoNexoPyme(form) {
  const lock = LockService.getScriptLock();
  lock.waitLock(30000);
  try {
    const data = normalizarPiloto_(form || {});
    validarPiloto_(data);
    ...
    const now = new Date();
    const clientId = generarClientIdPiloto_(data.negocio);
    ...
    const operational = crearPlanillaOperativaPiloto_(data, clientId, folderId);
    registrarConfigPiloto_(setupId, data, clientId, now);
    registrarPilotoComercial_(comercialId, {
      pilotoId,
    ...
    return {
      ok: true,
      version: NEXO_PILOT_GENERATOR_VERSION,
      pilotoId,
      clientId,
      negocio: data.negocio,
...
function normalizarPiloto_(form) {
  return {
...
function generarClientIdPiloto_(negocio) {
...
function crearPlanillaOperativaPiloto_(data, clientId, folderId) {
  const ss = SpreadsheetApp.create("NexoPyme Piloto - " + data.negocio + " - " + clientId);
  ss.setSpreadsheetTimeZone("America/Santiago");
  const first = ss.getSheets()[0];
  first.setName("CRM_VENTAS");

  Object.keys(NEXO_PILOT_HEADERS).forEach(function(name, index) {
    let sheet = index === 0 ? first : ss.insertSheet(name);
    const headers = NEXO_PILOT_HEADERS[name];
...
function registrarConfigPiloto_(setupId, data, clientId, now) {
...
function registrarPilotoComercial_(comercialId, p) {
  const ss = SpreadsheetApp.openById(comercialId);
  const sheet = ss.getSheetByName("PILOTOS");
  if (!sheet) throw new Error("No existe hoja PILOTOS en NexoPyme Comercial.");
...
function crear(){
 ...
 google.script.run.withSuccessHandler(function(r){b.disabled=false;b.textContent='Crear piloto 7 días'; const el=document.getElementById('result');el.style.display='block';el.innerHTML='<b>Piloto creado</b><br><br>CLIENT_ID: <code>'+r.clientId+'</code><br>Vence: '+new Date(r.vence).toLocaleString()+'<br><br><a target="_blank" href="'+r.operationalSheetUrl+'">Abrir planilla operativa</a><br><a target="_blank" href="'+r.setupProductosUrl+'">Cargar productos</a><br><a target="_blank" href="'+r.setupClientesUrl+'">Cargar clientes</a><br><a target="_blank" href="'+r.setupConexionUrl+'">Paso 4 · Conexión</a><br><br><b>Script Properties</b><br><code>CLIENT_ID='+r.scriptProperties.CLIENT_ID+'<br>SETUP_SPREADSHEET_ID='+r.scriptProperties.SETUP_SPREADSHEET_ID+'<br>OPERATIONAL_SPREADSHEET_ID='+r.scriptProperties.OPERATIONAL_SPREADSHEET_ID+'</code><br><br>'+r.siguientePaso;}).withFailureHandler(function(e){b.disabled=false;b.textContent='Crear piloto 7 días';const el=document.getElementById('error');el.style.display='block';el.textContent=e.message||String(e);}).crearPilotoNexoPyme({negocio:v('negocio'),contacto:v('contacto'),rut:v('rut'),razonSocial:v('razonSocial'),telefono:v('telefono'),email:v('email'),ciudad:v('ciudad'),dias:v('dias'),direccion:v('direccion'),observaciones:v('observaciones')});
}
</script></body></html>`;
}
